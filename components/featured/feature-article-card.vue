<template>
  <v-hover v-slot:default="{ hover }">
    <v-card class="ml-3 my-3 overallCard" :style="{'width':articleWidth+'%'}">
      <!-- image -->
      ...
      <v-card-text style="position: relative;" class="cardTextBlock">
        <!-- Bookmark -->
        ...
        <!-- Share -->
        <v-btn absolute color="whitesmoke" fab right top v-b-modal="type+article.url">
          <i class="fas fa-share-alt fa-lg"></i>
        </v-btn>
        <!-- Details -->
        <em class="font-weight-normal title mb-2 publisher" v-text="article.publisher"></em>
        <h3 class="mb-2 card-title text-left"
      ...
      <b-modal v-bind:id="type+article.url" header-bg-variant="primary"
               header-text-variant="light" style="z-index: 999">
...
<script>
    export default {
        name: "article-card",
        props: ['article', 'articleWidth', 'hoverText', 'type'],
        data() {
            return {
...
<style scoped>
  ...
  @media only screen and (max-width: 4096px) and (min-width: 1001px) {
    .articleButton {
      right: 35%;
    }
