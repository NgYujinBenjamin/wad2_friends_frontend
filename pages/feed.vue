      ...
      <div v-for="(article, index) in articles" :key="index">
        ...
        <b-card class="feed-card" :img-src="article.urlToImage" img-alt="Card image" img-top :title="article.title">
          ...
          <div v-for="language in languages">
            <button class="btn-outline-success" @click="translate(article.description,language.code)"><span
              v-text="language.desc"></span></button>
          </div>
        </b-card>
      </div>
    </masonry>
    ...
    export default {
        ...
        methods: {
            translate: function (description, language) {
                ...
                let url = 'https://api-apc.cognitive.microsofttranslator.com/translate?api-version=3.0&from=en&to=' + language;
                this.$axios.post(url,
                    [
                        {"text": description}
                    ], config
